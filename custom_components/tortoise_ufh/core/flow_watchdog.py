...
PHYSICAL witness of actuation; this module turns them into one per-loop
detection and one manual self-test:
...
The former stuck-open reverse detection (a closed COOLING valve still leaking
chilled water, witnessed by the room sitting below setpoint) was REMOVED
2026-07-13 (docs/DECISIONS.md §17): its room-air witness could not hard-verify
actuation, so it produced only false alarms. Hard actuation verification is
deferred to a future, dedicated mechanism.
...
_LOOP_STATUSES: tuple[str, ...] = ("ok", "no_flow", "inactive")
...
class LoopFlowMonitor:
    """Window machine of ONE loop's no-flow detection.

    Stateful: accumulates the no-flow window across cycles, holds the
    reference temperatures captured when the window opened, and remembers the
    last computed status. One instance per loop, owned by
    :class:`FlowWatchdog`.
    """

    def __init__(self) -> None:
        """Initialise an idle monitor (no window, ``"inactive"`` status)."""
    ...
    def restart(self) -> None:
        """Reset the window, references and flag (status back to inactive).
    ...
    def update(
        ...
        *,
        ...
        """Advance the loop's no-flow window by one control cycle.
        ...
        Args:
            ...
            circulation_evident: The building-level circulation gate:
                ...
                no-flow window (no accumulation, no reset).
        ...
        if commanded_pct >= open_threshold_pct:
            if supply_c is None or return_c is None:
                # No-flow needs the loop probes: a transiently stale probe
                # must not reset a 40-min window, so HOLD here and report
                # inactive.
                self._status = "inactive"
                return self._status
            ...
            # Command below the open threshold (closed or the middle zone):
            # nothing to expect from the loop, so reset the no-flow window
            # and report ok.
            self._reset_no_flow()
            self._status = "ok"
        return self._status
...
class FlowWatchdog:
    ...
    def update(
        ...
        *,
        ...
        Args:
            inputs: The room's raw inputs this cycle (loops, mode and the
                injected ``circulation_evident`` gate).
            commanded_pct: The valve position the core emitted on the
